...
class CursorSkillFetchTest < ActiveSupport::TestCase
  test "fetch-skills extras come from the plugin catalog without hardcoded extra URLs" do
    root = RecordingStudioOauth::Engine.root
  ...
  test "gem version stays 0.5.0 and gemspec still excludes .cursor" do
    assert_equal "0.5.0", RecordingStudioOauth::VERSION

    spec = Gem::Specification.load(RecordingStudioOauth::Engine.root.join("recording_studio_oauth.gemspec").to_s)
    cursor_files = spec.files.select { |path| path == ".cursor" || path.split("/").include?(".cursor") }

    assert_empty cursor_files

    tracked = `git -C #{RecordingStudioOauth::Engine.root} ls-files -- .cursor/rules`
