...
class HooksTest < Minitest::Test
  def test_template_does_not_ship_a_copied_hooks_class
    ...
    refute defined?(RecordingStudioOauth::Hooks)
  end
  ...
  def test_engine_runs_addon_hooks_through_configuration
    called = false
    RecordingStudioOauth.configuration.hooks.after_initialize { called = true }

    initializer = RecordingStudioOauth::Engine.initializers.find do |entry|
      entry.name == "recording_studio_oauth.after_initialize"
    end
