...
module RecordingStudioOauth
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      desc "Installs RecordingStudioOauth engine into your application"

      class_option(
        :mount_path,
        type: :string,
        default: "/recording_studio_oauth",
      ...
      def copy_initializer
        template "recording_studio_oauth_initializer.rb", "config/initializers/recording_studio_oauth.rb"
      end

      def add_yaml_config
        return unless yes?("Would you like to add `config/recording_studio_oauth.yml` for environment-specific settings? [y/N]")

        template "recording_studio_oauth.yml", "config/recording_studio_oauth.yml"
      end
      ...
      def add_tailwind_source
        ...
        if missing_lines.empty?
          say "Tailwind already configured to include RecordingStudioOauth and FlatPack sources.", :green
          return
        end
      ...
      def inject_tailwind_sources(tailwind_css_path, missing_lines)
        ...
        say "Added RecordingStudioOauth and FlatPack sources to Tailwind CSS configuration.", :green
        say "Run 'bin/rails tailwindcss:build' to rebuild your CSS.", :green
      end

      def formatted_tailwind_source_block(missing_lines)
        [
          "\n/* Include RecordingStudioOauth engine views for Tailwind CSS */",
      ...
      def tailwind_source_lines
        [
          '@source "../../vendor/bundle/**/recording_studio_oauth/app/views/**/*.erb";',
          '@source "../../../../../../usr/local/bundle/ruby/**/bundler/gems/recording_studio_oauth-*/app/views/**/*.erb";',
